...
class VelocityPIDController(PIDController):
    def __init__(self, kp, ki, kd, int_limit=None, sat_bound=None):
        super().__init__(kp, ki, kd, int_limit, sat_bound)
        # Inertia in NED body convention. The reference notebook hard-codes
        # the NWU form and applies a NWU<->NED round-trip around the velocity
        # PID call; since this repo operates in NED throughout, the off-diag
        # I_xy and I_xz are pre-flipped (R*I*R^T with R = diag(1,-1,-1)).
        self.mrb = np.array([
